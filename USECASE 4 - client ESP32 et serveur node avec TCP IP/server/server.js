...
//var address = process.env.HOST || '127.0.0.1'; //Il n'est pas bon de préciser l'adresse.
...
var server = net.createServer((socket)=>{
    ...
    socket.on('data', function(data) {
            ...
            log('DATA received from ' + socket.remoteAddress + ': ' + data);
            ...
            switch(jdata.action){
                case 'client-hello':                    
                    ...
                    socket.bannedGroups = []; //Ajoutons la liste des groupe desquels le client est banni
                ...
                case 'client-list-clients': //Envoi à l'envoyeur uniquement
                    ...
                    socket.write(JSON.stringify({from: jdata.from, action: 'server-list-clients', list: tab}));
                break;
                case 'client-quit': //Déconnexion normale
                    ...
                    socket.write(JSON.stringify({from: jdata.from, action: 'server-quit'}));
                ...
                case 'client-error': //Déconnexion forcée
                    ...
                    socket.write(JSON.stringify({from: jdata.from, code: jdata.code ,msg: jdata.msg, action:'server-error'}));
                ...
                case 'cgroup': //Création de groupe
                    ...
                    {
                        socket.write(JSON.stringify({group: jdata.group, action: 'group-already-exists'}));
                    }
                    else{
                ...
                case 'join': //Rejoindre un groupe
                    ...
                    {
                        socket.write(JSON.stringify({group: jdata.group, action: 'group-not-exists'}));
                    }
                    else{
                        if(!socketInGroup(socket, jdata.group)){ //Si le groupe existe et que le client n'y est pas encore...

                            if(!socketBannedFromGroup(socket, jdata.group)){ //Si le client n'est pas banni du groupe, on l'y ajoute
                            ...
                            else{//Si le client est banni du groupe
                                socket.write(JSON.stringify({group: jdata.group, action: 'can-not-join'}));
                            }
                ...
                //Si le groupe n'existe pas ou si le client n'y est pas
                    ...
                    {
                        socket.write(JSON.stringify({group: jdata.group, action: 'not-in-group'}));
                    }
                ...
                //Si le groupe n'existe pas ou si le client n'y est pas
                    ...
                    users.forEach((socketValue)=>{
                        ...
                        socket.write(JSON.stringify({from: jdata.from, group: jdata.group, list: tab, action: 'members'}));
                    }                
                break;
                ...
                //Si le groupe n'existe pas ou si le client n'y est pas
                    if(!groupExists(jdata.group) || !socketInGroup(socket, jdata.group))
                    {
                        ...
                        socket.write(JSON.stringify({from: jdata.from, group: jdata.group, action: 'msgs',
                            list: messagesOfGroup(jdata.group)}));
                    }                
                break;
                
                case 'groups': //Liste des groupes
                        socket.write(JSON.stringify({from: jdata.from, list: groups, action: 'groups'}));
                break;
                ...
                case 'invite': //Ajoute un client (invité) dans un groupe
                    ...
                    else if(!socketInGroup(socket, jdata.group))
                    {
                    ...
                    else{//Si l'envoyeur est dans le groupe

                        //Si celui qu'on veut ajouter n'est pas banni du groupe alors c'est bon
                        if(!socketBannedFromGroup(getSocketByName(jdata.dest), jdata.group)){
                        ...
                        else{//Si celui qu'on veut ajouter est banni du groupe
                            socket.write(JSON.stringify({group: jdata.group, dest: jdata.dest, action: 'can-not-invite'}));
                        }
                ...
                case 'kick': //Retirer quelqu'un d'un groupe
                    ...
                    {
                        socket.write(JSON.stringify({group: jdata.group, dest: jdata.dest, action: 'dest-not-in-group'}));
                    }
                ...
                case 'ban': //Bannir quelqu'un d'un groupe
                    //Si le groupe n'existe pas ou si l'envoyeur n'y est pas
                    if(!groupExists(jdata.group) || !socketInGroup(socket, jdata.group))
                    {
                        socket.write(JSON.stringify({group: jdata.group, action: 'not-in-group'}));
                    }
                    else if(!socketInGroup(getSocketByName(jdata.dest), jdata.group)) //Si celui qu'on veut bannir n'y est pas
                    {
                        socket.write(JSON.stringify({group: jdata.group, dest: jdata.dest, action: 'dest-not-in-group'}));
                    }
                    else{//Le groupe existe, l'envoyeur et le dest y sont: on peut procéder
                        myData = {from: jdata.from, group: jdata.group, dest: jdata.dest, action: 'ban', reason: jdata.reason}
                        sendToGroup(myData, jdata.group);
                        sentMessages.push(myData);
                        bd_addMessage(myData);
                        ...
                        removeFromGroup(jdata.group, jdata.dest);
                        bd_removeFromGroup(jdata.dest, jdata.group);
                        banFromGroup(jdata.group, jdata.dest);  //On le bannit du groupe
                        bd_banFromGroup(jdata.dest, jdata.group);
                    }
                break;

                case 'unban': //Débannir quelqu'un d'un groupe
                    //Si le groupe n'existe pas ou si l'envoyeur n'y est pas
                    if(!groupExists(jdata.group) || !socketInGroup(socket, jdata.group))
                    {
                        socket.write(JSON.stringify({group: jdata.group, action: 'not-in-group'}));
                    }
                    else if(!socketBannedFromGroup(getSocketByName(jdata.dest), jdata.group)) //Si celui qu'on veut débannir n'est pas banni
                    {
                        socket.write(JSON.stringify({group: jdata.group, dest: jdata.dest, action: 'dest-not-banned'}));
                    }
                    else{//Si celui qu'on veut débannir est banni: on peut procéder
                        myData = {from: jdata.from, group: jdata.group, dest: jdata.dest, action: 'unban'}
                        sendToGroup(myData, jdata.group);
                        sentMessages.push(myData);
                        bd_addMessage(myData);

                        unbanFromGroup(jdata.group, jdata.dest);  //On le débannit du groupe
                        bd_unbanFromGroup(jdata.dest, jdata.group);
                    }
                break;
                ...
                case 'states': //Lister tous les événements survenus dans le groupe
                    ...
                    if(!groupExists(jdata.group) || !socketInGroup(socket, jdata.group))
                    {
                        ...
                        socket.write(JSON.stringify({from: jdata.from, group: jdata.group, action: 'states',
...
function socketBannedFromGroup(a_socket, groupName){ //True si a_socket est banni du groupe groupName

    let tab = a_socket.bannedGroups || []; //Tableau des groupes desquels ce socket est banni
...
function sendToOne(data, toName){ //Envoi d'un message à  un client précis
    ...
    users.forEach((socketValue)=>{
        ...
        {
            socketValue.write(JSON.stringify(data));
        }
    });
}
...
function banFromGroup(a_group, a_user){ //Bannit un client d'un groupe
    if(a_socket = getSocketByName(a_user)){
        a_socket.bannedGroups.push(a_group);
    }
    }

function unbanFromGroup(a_group, a_user){ //Débannit un client d'un groupe

    if(a_socket = getSocketByName(a_user)){
        a_socket.bannedGroups = a_socket.bannedGroups.filter((value)=>{
            return !lazyEqual(value, a_group);
        })
    }
}
...
function recreateSocketGroups(a_socket){ //Associe le socket aux groupes dont il fait partie, à partir de la BD
    db.serialize(() => {
        ...
        WHERE username = ?`, [a_socket.name], (err, rows) => {
          ...
          recreateSocketBans(a_socket).then(
            ()=>{
...
function recreateSocketBans(a_socket){ //Associe le socket aux groupes dont il a été banni, à partir de la BD
    return new Promise((resolve, reject)=>{
        db.serialize(() => {
            db.all(`SELECT DISTINCT groupname FROM bans 
            WHERE username = ?`, [a_socket.name], (err, rows) => {
              if (err) {
                console.error(err.message); reject(err);
              }
              rows.forEach((row)=>{
                a_socket.bannedGroups.push(row.groupname);
              });
              resolve('récupération des bans ok.');
            });
          });
    });
}
...
function recreateSocketSession(a_socket, action){ //Récupère la session d'un client (dans un tableau de messages)
    ...
    let tab = [], msgList = []; let req = ''; console.log('GROUPES', a_socket.groups);
    ...
    db.serialize(() => {
        ...
        db.all(req, [a_socket.name, a_socket.name, a_socket.name], (err, rows) => {
          ...
          if(msgList.length>0){
            a_socket.write(JSON.stringify({from: a_socket.name, list: msgList, action: action}));
          }
...
function bd_banFromGroup(userName, groupName){ //Retire un user d'un groupe
    db.run(`INSERT INTO bans([groupname], [username]) VALUES(?, ?)`,
    [groupName, userName],
    function(err) {
        if (err) {
          return console.log(err.message);
        ...
        // get the last insert id
        console.log(`Table bans: A row has been inserted with rowid ${this.lastID}`);
      });
}

function bd_unbanFromGroup(userName, groupName){ //Retire un user d'un groupe
    db.run(`DELETE FROM bans WHERE username=? AND groupname=?`,
    [userName, groupName],
    function(err) {
        if (err) {
          return console.log(err.message);
        ...
        // get the last insert id
        console.log(`Table bans: A row has been deleted with rowid ${this.lastID}`);
      });
}
          ...
          if(rows.length > 0){//Si l'utilisateur existe en BD, on vérifie son mot de passe
            ...
            bcrypt.compare(a_socket.pass, rows[0].password, function(err, result) {
                ...
                if(result){//Bon mot de passe
                    ...
                    a_socket.write(JSON.stringify({from: a_socket.name, action:'unknown-user'}));
                }
            });
          }
 ...
 */
    ...
    socket.write(JSON.stringify({from: socket.name, action:'server-hello',  msg: 'Welcome '+socket.name+' !'}));
